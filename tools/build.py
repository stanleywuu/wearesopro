...
START = "<!-- nav:start (generated by tools/build.py — do not edit by hand) -->"
END = "<!-- nav:end -->"


def nav_html():
    links = "\n".join(
        '  <a href="/%s">%s</a>' % (path, label)
        for path, label, in_nav in PAGES if in_nav
    )
    return '%s\n<nav class="site-links" aria-label="All pages">\n%s\n</nav>\n%s\n' % (
        START, links, END)


def insert(source, block):
